...
- 版面（真牌桌三区）：上方比分徽章＋連勝/最佳/場次/烏龜；牌桌內**頂部**＝AI pod 列（頭像＋依 history 的表情＋牌背小扇＋張數，開追蹤後秀最近 3 張被抽走牌的小徽章）、**中央**＝公開丟牌堆（重疊微旋轉＋已丟計數章）、**底部**＝玩家手牌真扇形（CSS 向量亮牌、按住抬起）；抽牌結果以置中揭示卡演出（3D 翻面 `.flip/.flip-inner/.show`，同揭示去重；成對時雙卡並列）。
- 張力與演出：剩牌 ≤3 張時 `body[data-tense]` 紅框脈動＋揭示放大＋心跳音（`audio.heartbeat`）＋觸感（`navigator.vibrate` 守衛生）；結算逃脫撒 CSS 彩帶、中烏龜則 🐢 爬過；`prefers-reduced-motion` 全部退化。
...
4. ✅ **盲抽手感**：揭示改真 3D 翻面（`.flip/.flip-inner`＋`.show`，同揭示去重不重建）；成對時抽出牌與配對手牌並列；尊重 `prefers-reduced-motion`。
5. ✅ **記憶輔助**：AI 各家下方「曾被抽走」小徽章（`drawnFromHistory`＋測試），👁 追蹤鈕可開關（預設開，僅存記憶體）。
6. ✅ **音效偏好持久化**：靜音狀態存進 `pg-zhuagui-stats.muted`，切換立即 PUT、啟動時套用。

**低優先**

7. ✅ **牌面清晰度**：正面改 CSS 向量繪製（`.css-card`：角落點數＋花色、中央大花色、鬼牌 🐢、紅黑配色、`--cw` 隨版面縮放），任何解析度都銳利；背面維持 Kenney PNG（`assets/cards/card_back.png`）。連帶完成三区牌桌版面重構（`index.html`/`styles.css`/`app.js` 渲染層；`game.js` 核心邏輯與測試未動，37 例維持綠燈）。
